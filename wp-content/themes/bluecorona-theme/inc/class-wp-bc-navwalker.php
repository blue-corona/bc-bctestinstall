...
if ( ! class_exists( 'Bluecorona_WP_Bootstrap_Navwalker' ) ) {
	...
	class Bluecorona_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$classes = array( 'dropdown-menu', 'bc_hero_bg_form_color' );
			/**
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				...
				$atts['class']         = 'dropdown-toggle nav-link text-white mr-4';
				$atts['id']            = 'menu-item-dropdown-' . $item->ID;
			} else {
				$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
				// For items in dropdowns use .dropdown-item instead of .nav-link.
				if ( $depth > 0 ) {
					$atts['class'] = 'dropdown-item text-white mr-4';
				} else {
					$atts['class'] = 'nav-link text-white mr-4';
				}
			}
			...
			$icon_html = '';
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				$icon_html = '&nbsp;<i class="fa fa-angle-down bc_menu_icon_color" aria-hidden="true"></i> ';
			} else {
